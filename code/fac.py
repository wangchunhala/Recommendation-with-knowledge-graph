...
def user_attention(user_latent, path_output,item_latent):
    latent_size = user_latent.shape[1].value

    inputs = concatenate([user_latent, path_output,item_latent])
...
def item_attention(item_latent, path_output,user_latent):
...
def path_attention1(item_latent, path_output, user_latent):
    latent_size = path_output.shape[1].value

    inputs = concatenate([item_latent, path_output, user_latent])
    output = Dense(latent_size,
                   activation='relu',
                   kernel_initializer='glorot_normal',
                   kernel_regularizer=l2(0.001),
                   name='path_attention_layeradd')(inputs)
    atten = Lambda(lambda x: K.softmax(x), name='path_attention_softmaxadd')(output)
    output = multiply([path_output, atten])
    return output
...
def get_model(usize, isize, path_nums, timestamps, length, layers = [20, 10], reg_layers = [0, 0], latent_dim = 40, reg_latent = 0):
    ...
    user_atten = user_attention(user_latent, path_output,item_latent)
    item_atten = item_attention(item_latent, path_output,user_latent)
    path_output = path_attention1(item_latent, path_output, user_latent)
